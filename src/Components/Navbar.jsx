import React, { use } from 'react';
import { NavLink, useNavigate } from 'react-router';
import AuthContext from '../Contexts/AuthContext';

const Navbar = () => {

    const navigate = useNavigate();

    const { user } = use(AuthContext);
    console.log(user);

    const handleProfileIcon = () => {
        navigate('/profile');
    }

    const handleLoginButton = () => {
        navigate('/login');
    }
    const handleLogOutButton = () => {
        console.log('user want to logout');
    }

    const links = <>
        <NavLink to={'/'}>Home</NavLink>
        <NavLink to={'/toys'}>Toys</NavLink>
        <NavLink to={'/profile'}>My Profile</NavLink>
        <NavLink to={'/about'}>About</NavLink>
    </>

    return (
        <div>
            <div className="navbar px-[5%] bg-base-100 shadow-sm">
                <div className="navbar-start">
                    <div className="dropdown">
                        <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /> </svg>
                        </div>
                        <ul
                            tabIndex="-1"
                            className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
                            {links}
                        </ul>
                    </div>
                    <div className='flex space-x-1.5'>
                        <img className='h-12' src="/Logo.png" alt="Company Logo" />
                        <a className="btn btn-ghost pl-0 my-auto text-xl">BD Toys</a>
                    </div>
                </div>
                <div className="navbar-center hidden lg:flex">
                    <ul className="menu menu-horizontal px-1 space-x-3.5">
                        {links}
                    </ul>
                </div>
                <div className="navbar-end space-x-3">
                    <div tabIndex={0} role="button" className="btn btn-ghost btn-circle avatar">
                        <div className="w-10 rounded-full">
                            <img
                                onClick={handleProfileIcon}
                                alt="Tailwind CSS Navbar component"
                                src="https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp" />
                        </div>
                    </div>
                    {
                        user ? (
                            <a onClick={handleLogOutButton} className='btn'>LogOut</a>
                        ) : (
                            <a onClick={handleLoginButton} className="btn">Login</a>
                        )
                    }
                </div>
            </div>
        </div>
    );
};

export default Navbar;